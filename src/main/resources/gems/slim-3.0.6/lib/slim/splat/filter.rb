    ...
    class Filter < ::Slim::Filter
      ...
      def call(exp)
        ...
        if @splat_options
          ...
          [:multi, [:sc, "#{@splat_options} = #{opts}"], exp]
        else
          exp
        end
      end
      ...
      def make_builder(attrs)
        ...
        builder = unique_name
        result = [:multi, [:sc, "#{builder} = ::Slim::Splat::Builder.new(#{@splat_options})"]]
        attrs.each do |attr|
          result <<
            if attr[0] == :html && attr[1] == :attr
              if attr[3][0] == :slim && attr[3][1] == :attrvalue
                [:sc, "#{builder}.code_attr(#{attr[2].inspect}, #{attr[3][2]}, (#{attr[3][3]}))"]
              else
                tmp = unique_name
                [:multi,
                 [:capture, tmp, compile(attr[3])],
                 [:sc, "#{builder}.attr(#{attr[2].inspect}, #{tmp})"]]
              end
            elsif attr[0] == :slim && attr[1] == :splat
              [:sc, "#{builder}.splat_attrs((#{attr[2]}))"]
            else
              attr
            end
        end
